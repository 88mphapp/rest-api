...
const getRawPoolList = (version: number) => {
  let poolListPath;
  switch (version) {
    case 2:
...
const getPoolInfo = (name: string, version: number = 3): PoolInfo => {
  return getRawPoolList(version)[name];
};

const getPoolInfoList = (version: number = 3): PoolInfo[] => {
  return Object.keys(getRawPoolList(version)).map((pool) => getPoolInfo(pool, version));
};

const getPoolInfoFromAddress = (address: string, version: number = 3): PoolInfo => {
    ...
    if (dpools) {
      ...
      await Promise.all(
        dpools.map(async (pool) => {
          ...
          const totalValueLockedInToken = new BigNumber(pool.totalDeposit);
          const totalValueLockedInUSD =
            totalValueLockedInToken.times(stablecoinPrice);
